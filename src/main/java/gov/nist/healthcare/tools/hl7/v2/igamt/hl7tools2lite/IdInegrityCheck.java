...
// It compares the state of id values in the IGAMT dataset both before and after the
// HL7Tools2LiteConverter has been run.
// To do this, one must start with two identical copies of the IGAMT dataset, run the
// HL7Tools2LiteConverter against one, then
// run IdInegrityCheck against the two. In this context, "from" means the unaltered dataset and "to"
// means the altered or converted
// dataset.
// This class produces output based on the "no news is good news philosophy" Only errors and
...
public class IdInegrityCheck implements Runnable {
  ...
  @Option(name = "-f", aliases = "--from", required = true,
      usage = "Name of the database we compare from.")
  String dbFrom;

  @Option(name = "-t", aliases = "--to", required = true,
      usage = "Name of the database we compare to.")
  String dbTo;
  ...
  @Option(name = "-v", aliases = "--versions", handler = StringArrayOptionHandler.class,
      required = true,
      usage = "Blank delimited set of hl7Versions for which to process. e.g. 2.5.1 2.6 2.7")
  String[] hl7Versions;
  ...
  public int checkFields4Tables(Segment to) {
    ...
    for (Field fld : to.getFields()) {
      tablesInFields += getNumberOfTables(to.getValueSetBindings(), fld.getPosition());
    }
    return tablesInFields;
  }

  public int getNumberOfTables(List<ValueSetOrSingleCodeBinding> bindings, int position) {
    int i = 0;
    for (ValueSetOrSingleCodeBinding binding : bindings) {
      if (binding.getLocation().equals(position + "")) {
        i++;
      }
    }
    return i;
  ...
  public int checkComponents4Tables(Datatype to) {
    ...
    for (Component comp : to.getComponents()) {
      tablesInComponents += getNumberOfTables(to.getValueSetBindings(), comp.getPosition());
    }
  ...
  public void checkTables(String hl7Version) {
    ...
    while (itrTabsFrom.hasNext() && itrTabsTo.hasNext()) {
      ...
      int[] match = validate(from.getBindingIdentifier(), to.getBindingIdentifier(), from.getId(),
          to.getId());
      while (match[0] != 0) {
        if (match[0] < 0) {
        ...
        match = validate(from.getBindingIdentifier(), to.getBindingIdentifier(), from.getId(),
            to.getId());
      }
      if (match[1] != 0) {
        log.error("ObjectId mismatch: from=" + from.getBindingIdentifier() + " to="
  ...
  public void checkTableLibrary(String hl7Version) {
    ...
    for (TableLink link : lib.getChildren()) {
      if (findTableById(mongoTo, link.getId()) == null) {
        ...
        // List<Table> tabs = findTableByBindingId(hl7Version, mongoTo,
        // link.getBindingIdentifier());
        // if (tabs.size() > 0) {
  ...
  List<Message> findMessages(MongoOperations mongoOps, String hl7Version) {
    Criteria where =
        Criteria.where("scope").is(Constant.SCOPE.HL7STANDARD).and("hl7Version").is(hl7Version);
    Query qry = Query.query(where);
  ...
  List<Table> findTableByBindingId(String hl7Version, MongoOperations mongoOps, String id) {
    Criteria where = Criteria.where("scope").is(Constant.SCOPE.HL7STANDARD).and("hl7Version")
        .is(hl7Version).and("bindingIdentifier").is(id);
    Query qry = Query.query(where);
