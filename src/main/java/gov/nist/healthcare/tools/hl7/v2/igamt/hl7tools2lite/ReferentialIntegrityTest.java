...
public class ReferentialIntegrityTest {
	...
	public boolean account4Events(Profile profile) {
		...
		Set<String> vsStructIds = new HashSet<String>();
		for (Table valueSet : tables.getChildren()) {
			if("0354".equals(valueSet.getBindingIdentifier())) {
				for (Code code : valueSet.getCodes()) {
					vsStructIds.add(code.getValue());
				}
			}
		}
	...
	public boolean account4ValueSets(Profile profile) {
		...
		for (Datatype dt : datatypes.getChildren()) {
			for (Component cmp : dt.getComponents()) {
				if (cmp.getTable() != null && cmp.getTable().trim().length() > 0) {
	...
	boolean account4DataTypes(Profile profile) {
		...
		return (hasNotsegDtIds + hasNotdtIds) == 0;
	}

	public boolean account4Segments(Profile profile) {
		SegmentLibrary segs = profile.getSegmentLibrary();
		...
		return (hasNotsegIds.size() + hasNotsegRefs.size()) == 0;
	}
	...
	public void run() {
		...
		try {
			...
			while (cur.hasNext()) {
				...
				b = account4ValueSets(igdocument.getProfile());		
			}
		} catch (Exception e) {
			log.error("", e);
		}
