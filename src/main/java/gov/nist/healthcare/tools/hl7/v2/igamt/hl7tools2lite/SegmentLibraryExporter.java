...
public class SegmentLibraryExporter {

	private static final Logger log = LoggerFactory.getLogger(SegmentLibraryExporter.class);

	public final static File OUTPUT_DIR_LOCAL = new File(System.getenv("LOCAL_OUT") + "/segmentLibraries");
	public final static File OUTPUT_DIR_IGAMT = new File(System.getenv("IGAMT_OUT") + "/segmentLibraries");
	...
	public void run() {
		...
		DBCollection coll = mongoOps.getCollection("segment-library");
		...
		while (cur.hasNext()) {
			...
			File outfileLocal = new File(OUTPUT_DIR_LOCAL , "segLib-" + lib.getScope().name() + "-" + hl7Version + ".json");
			File outfileIGAMT = new File(OUTPUT_DIR_IGAMT , "segLib-" + lib.getScope().name() + "-" + hl7Version + ".json");
			try {
	...
	public static void main(String[] args) throws Exception {
		SegmentLibraryExporter app = new SegmentLibraryExporter();
		app.run();
	}
}