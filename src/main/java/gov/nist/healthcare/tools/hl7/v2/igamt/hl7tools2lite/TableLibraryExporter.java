...
public class TableLibraryExporter {
	...
	public void run() {
		...
		DBCollection coll = mongoOps.getCollection("table-library");
		...
		while (cur.hasNext()) {
			...
			File outfileLocal = new File(OUTPUT_DIR_LOCAL , "tabLib-" + lib.getScope().name() + "-" + hl7Version + ".json");
			File outfileIGAMT = new File(OUTPUT_DIR_IGAMT , "tabLib-" + lib.getScope().name() + "-" + hl7Version + ".json");
			try {
